...
import (
	"database/sql"
	"errors"
	"github.com/sindhutrisha/intelops_compage_testing/code/pkg/rest/server/daos/clients/sqls"
	"github.com/sindhutrisha/intelops_compage_testing/code/pkg/rest/server/models"
...
func (uniqueDao *UniqueDao) CreateUnique(m *models.Unique) (*models.Unique, error) {
	insertQuery := "INSERT INTO uniques(Age, Unique, Verified)values(?, ?, ?)"
	res, err := uniqueDao.sqlClient.DB.Exec(insertQuery, m.Age, m.Unique, m.Verified)
	if err != nil {
		return nil, err
	}
...
func (uniqueDao *UniqueDao) UpdateUnique(id int64, m *models.Unique) (*models.Unique, error) {
	...
	updateQuery := "UPDATE uniques SET Age = ?, Unique = ?, Verified = ? WHERE Id = ?"
	res, err := uniqueDao.sqlClient.DB.Exec(updateQuery, m.Age, m.Unique, m.Verified, id)
	if err != nil {
		return nil, err
	}
...
func (uniqueDao *UniqueDao) ListUniques() ([]*models.Unique, error) {
	...
	for rows.Next() {
		m := models.Unique{}
		if err = rows.Scan(&m.Id, &m.Age, &m.Unique, &m.Verified); err != nil {
			return nil, err
		}
...
func (uniqueDao *UniqueDao) GetUnique(id int64) (*models.Unique, error) {
	...
	m := models.Unique{}
	if err := row.Scan(&m.Id, &m.Age, &m.Unique, &m.Verified); err != nil {
